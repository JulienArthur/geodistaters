...
#API pour géocoder : https://operations.osmfoundation.org/policies/nominatim/
...
# Par exemple à remplacer par la base de données:
df <- data.frame(location = c("Paris", "Berlin", "Rome"))
...
get_coordinates <- function(location) {
  response <- GET(paste0("https://nominatim.openstreetmap.org/search?q=", URLencode(location), "&format=json"),
                  add_headers(`User-Agent` = "MonApplication/1.0 (email@example.com)"))
